...
class AllDataPairDataset(Dataset):
    def __init__(self, csv_file, device='cuda', normalize=True, augment=False, 
        ...
        # Add after self.array_features definition
        ...
        self.one_hot_features = self.joint_features + self.movement_features

        print(f"Found {len(self.joint_features)} joint features and {len(self.movement_features)} movement features")
    ...
    def extract_features(self, video_name):
        ...
        if video_data.empty:
            total_features = len(self.array_features) * 2 + len(self.one_hot_features)
            return torch.zeros((self.mean_sequence_length, total_features), dtype=torch.float32)
        
        # Extract array features (same as before)
        array_features_list = []
        for feature in self.array_features:
        ...
        # Extract one-hot features and repeat for each timestep
        one_hot_values = []
        for feature in self.one_hot_features:
            if feature in video_data.columns:
                value = video_data[feature].values[0] if len(video_data) > 0 else 0.0
                one_hot_values.append(float(value) if not pd.isna(value) else 0.0)
            else:
                one_hot_values.append(0.0)
        
        # Repeat one-hot features for each timestep
        one_hot_array = np.tile(one_hot_values, (self.mean_sequence_length, 1)).astype(np.float32)
        
        # Concatenate array features with one-hot features
        all_features = np.concatenate([all_features, one_hot_array], axis=1)
...
def log_gradient_norms(model, writer, epoch):
    """Log gradient norms to help debug gradient issues"""
    total_norm = 0
    param_count = 0
    for name, param in model.named_parameters():
        if param.grad is not None:
            ...
            param_count += 1
            writer.add_scalar(f'Gradient_Norms/{name}', param_norm, epoch)
...
def train_enhanced_siamese_model(csv_path, batch_size=16, num_epochs=6, learning_rate=0.0001, 
    ...
    for epoch in range(num_epochs):
        ...
        # Replace the training loop with mixed precision
