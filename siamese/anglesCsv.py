...
def get_wrist_angle(frames, movement_start_frame, movement_end_frame):
    ...
    for frame_index in range(movement_start_frame, movement_end_frame):
        ...
        left_elbow = get_xyz(frame[6])
        left_wrist = get_xyz(frame[7])
...
count1 = 0
def export_features_to_csv(video_name, joint, angles, angular_velocities, angular_accelerations, angles2, angular_velocities2, angular_accelerations2,
                            angles3, angular_velocities3, angular_accelerations3, angles4, angular_velocities4, angular_accelerations4, 
                            label, frequency_domain, biomechanical_metrics, distances, velocities, accelerations, master_file):
    global count1
    scalar_data = {
    ...
    angle_features = {
        'angle': json.dumps(angles),  # Convert to JSON string
        'angular_velocity': json.dumps(angular_velocities),
        'angular_acceleration': list(angular_accelerations),
        'angle2': json.dumps(angles2),
        'angular_velocity2': json.dumps(angular_velocities2),
        'angular_acceleration2': list(angular_accelerations2),
        'angle3': json.dumps(angles3),
        'angular_velocity3': json.dumps(angular_velocities3),
        'angular_acceleration3': list(angular_accelerations3),
        'angle4': json.dumps(angles4),
        'angular_velocity4': json.dumps(angular_velocities4),
        'angular_acceleration4': list(angular_accelerations4),
        'label': label,
    }
    ...
    for i, key in enumerate(distances.keys()):
        if i > 0:
            break
        movement_features[f"distance{i+1}"] = distances[key]
        movement_features[f"velocity{i+1}"] = velocities[key]
        movement_features[f"acceleration{i+1}"] = accelerations[key]
...
if os.path.exists(folder_name) and os.path.isdir(folder_name):  #
    ...
    for video_name in os.listdir(folder_name):     # angle1 missing                                                 # not fully loaded
        ...
        if "BACK" in video_name:
            ...
            angles2, angular_velocities2, angular_accelerations2, frequency_domain2, biomechanical_metrics2 = get_shoulder_front_angle(frames, movement_start_frame, movement_end_frame, True)
            ...
            if movement_start_end_frames.get("ELBOW__RAISE_ARMS_IN_FRONT_AND_FLEX_ELBOWS__BOTH", None) is not None:
                ...
                angles2, angular_velocities2, angular_accelerations2, frequency_domain2, biomechanical_metrics2 = get_shoulder_front_angle(frames, movement_start_end_frames["ELBOW__RAISE_ARMS_IN_FRONT_AND_FLEX_ELBOWS__BOTH"][0], movement_start_end_frames["ELBOW__RAISE_ARMS_IN_FRONT_AND_FLEX_ELBOWS__BOTH"][1], True)
                angles3, angular_velocities3, angular_accelerations3, frequency_domain3, biomechanical_metrics3 = get_elbow_angle(frames, movement_start_end_frames["ELBOW__RAISE_ARMS_IN_FRONT_AND_FLEX_ELBOWS__BOTH"][0], movement_start_end_frames["ELBOW__RAISE_ARMS_IN_FRONT_AND_FLEX_ELBOWS__BOTH"][1], False)
                angles4, angular_velocities4, angular_accelerations4, frequency_domain4, biomechanical_metrics4 = get_shoulder_front_angle(frames, movement_start_end_frames["ELBOW__RAISE_ARMS_IN_FRONT_AND_FLEX_ELBOWS__BOTH"][0], movement_start_end_frames["ELBOW__RAISE_ARMS_IN_FRONT_AND_FLEX_ELBOWS__BOTH"][1], False)
            ...
            angles, angular_velocities, angular_accelerations, frequency_domain, biomechanical_metrics = get_wrist_angle(frames, movement_start_end_frames["WRISTS__BEND_LEFT_AND_RIGHT__BOTH"][0], movement_start_end_frames["WRISTS__BEND_LEFT_AND_RIGHT__BOTH"][1])
        elif "LEFT_WRIST" in video_name:
            angles, angular_velocities, angular_accelerations, frequency_domain, biomechanical_metrics = get_wrist_angle(frames, movement_start_end_frames["WRIST__BEND_LEFT_AND_RIGHT__LEFT"][0], movement_start_end_frames["WRIST__BEND_LEFT_AND_RIGHT__LEFT"][1])
        ...
        velocity_data = {
            ...
            "angular_velocities2": angular_velocities2,
            "angular_velocities3": angular_velocities3,
            "angular_velocities4": angular_velocities4,
        }

        acceleration_data = {
            "angular_accelerations": angular_accelerations,
            "angular_accelerations2": angular_accelerations2,
            "angular_accelerations3": angular_accelerations3,
            "angular_accelerations4": angular_accelerations4,
        }

        # Find a valid reference length
        reference_length = next(
            (len(lst) for lst in angle_data.values() if lst), 
            0  # Default to 0 if all lists are empty
        )

        # If all angles are empty, skip processing this video
        if reference_length == 0:
            missing_ang.append(stripped_video_name)
            continue

        # Fill missing lists with -999
        for key in angle_data:
            if not angle_data[key]:
                angle_data[key] = [-1] * reference_length

        for key in velocity_data:
            if not velocity_data[key]:
                velocity_data[key] = [-1] * reference_length

        for key in acceleration_data:
            if acceleration_data[key].size == 0:
                acceleration_data[key] = np.array([-1] * reference_length)
        ...
        export_features_to_csv(
            stripped_video_name, joint_name, 
            angle_data["angles"], velocity_data["angular_velocities"], acceleration_data["angular_accelerations"],
            angle_data["angles2"], velocity_data["angular_velocities2"], acceleration_data["angular_accelerations2"],
            angle_data["angles3"], velocity_data["angular_velocities3"], acceleration_data["angular_accelerations3"],
            angle_data["angles4"], velocity_data["angular_velocities4"], acceleration_data["angular_accelerations4"],
            label, frequency_domain, biomechanical_metrics, distances, velocities, accelerations, master_file
        )
